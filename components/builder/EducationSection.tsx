    ...
    <>
      <div className="space-y-2">
        {data.map((edu, i) => (
          <div
            key={i}
            className="group relative flex items-start justify-between p-3.5 sm:p-4 rounded-xl border border-zinc-800/60 bg-zinc-900/40 hover:bg-zinc-900/80 hover:border-amber-500/30 cursor-pointer transition-all duration-200"
          ...
          >
            <div className="flex items-start gap-3 flex-1 min-w-0 pr-2">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-zinc-100 leading-tight truncate">{edu.degree}</p>
                <p className="text-xs text-zinc-400 mt-0.5 truncate">{edu.institute}</p>
      ...
      {deleteIndex !== null && (
        <DeleteConfirmModal
          ...
          title="Delete Education"
          itemName={data[deleteIndex]?.degree || data[deleteIndex]?.institute}
          onConfirm={() => {
            if (deleteIndex !== null) {
